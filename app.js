    ...
    if (courseNameColumn) {
      ...
      return `
        ...
        <section class="toolbar subject-toolbar" aria-label="과목 검색과 개설 교과 필터">
          ...
          <button class="school-course-toggle ${state.schoolOnlyCourses ? "is-active" : ""}" type="button" data-school-course-toggle aria-pressed="${state.schoolOnlyCourses}" ${state.curriculum ? "" : "disabled"} title="${state.curriculum ? `${escapeHtml(state.selectedSchool?.name || "선택 학교")}의 개설 교과만 보기` : "먼저 편제표가 연동된 학교를 선택해 주세요."}"><span class="school-course-toggle-title">${icon("check")}<strong>개설 교과</strong></span>${state.selectedSchool?.name ? `<small class="school-course-school-badge">${icon("school")}<b>${escapeHtml(state.selectedSchool.name)}</b></small>` : '<small class="school-course-school-badge is-empty">학교 선택 필요</small>'}</button>
        </section>
        ${resultsMarkup}`;
    }
