(async () => {
  ...
  const curriculumLeaveDialog = document.querySelector("#curriculum-leave-dialog");
  const toast = document.querySelector("[data-app-toast]");
  let curriculumAlertConfirmAction = null;
  let curriculumLeaveConfirmAction = null;
  ...
  const state = {
    ...
    curriculumSavedFingerprints: {},
    curriculumPreviewIndex: 0,
    curriculumPreviewGradeIndex: 0,
    curriculumYearPickerOpen: false,
  ...
  function dialogFooterMarkup() {
    return `<div class="major-dialog-footer dialog-action-footer"><button type="button" data-dialog-close>닫기</button></div>`;
  }
  ...
  function localizedAccessError(error, context = "general") {
    ...
    if (/[A-Za-z]{3,}/.test(message)) {
      ...
      if (context === "upload") return "업로드 파일을 확인하지 못했습니다. 2025·2026학년도 파일과 파일 형식을 다시 확인해 주세요.";
  ...
  function showCurriculumAlert(title, message, kicker = "데이터 확인 안내", variant = "warning", confirmAction = null) {
    if (!curriculumAlertDialog || !curriculumAlertTitle || !curriculumAlertMessage) return;
    const isSuccess = variant === "success";
    curriculumAlertConfirmAction = typeof confirmAction === "function" ? confirmAction : null;
    const kickerNode = curriculumAlertDialog.querySelector(".curriculum-alert-kicker");
    const symbolUse = curriculumAlertDialog.querySelector(".curriculum-alert-symbol use");
    curriculumAlertDialog.classList.toggle("is-success", isSuccess);
    if (kickerNode) kickerNode.textContent = kicker;
    if (symbolUse) symbolUse.setAttribute("href", `icons.svg#${isSuccess ? "check" : "warning"}`);
    curriculumAlertTitle.textContent = compactText(title) || "편제표를 확인해 주세요";
    curriculumAlertMessage.textContent = displayValue(message).split(/\r?\n/).map(compactText).filter(Boolean).join("\n") || "편제표 내용을 확인한 뒤 다시 시도해 주세요.";
  ...
  function completeSchoolName(value) {
    const prefix = schoolNamePrefix(value);
    return prefix ? `${prefix}고등학교` : "";
  }

  function stableCurriculumValue(value) {
    if (Array.isArray(value)) return value.map(stableCurriculumValue);
    if (!value || typeof value !== "object") return value;
    return Object.keys(value).sort().reduce((result, key) => {
      if (value[key] !== undefined) result[key] = stableCurriculumValue(value[key]);
      return result;
    }, {});
  }

  function curriculumEditFingerprint(curriculum) {
    return JSON.stringify(stableCurriculumValue({
      schoolName: compactText(curriculum?.schoolName),
      region: compactText(curriculum?.region),
      admissionYear: Number(curriculum?.admissionYear) || null,
      grades: Array.isArray(curriculum?.grades) ? curriculum.grades : [],
      courseMetadata: curriculum?.courseMetadata && typeof curriculum.courseMetadata === "object" ? curriculum.courseMetadata : {}
    }));
  }

  function markPendingCurriculaSaved(curriculumIndexes = null) {
    const curricula = preparePendingCurriculumForEditing();
    const requestedIndexes = Array.isArray(curriculumIndexes) ? new Set(curriculumIndexes.map(Number)) : null;
    const nextFingerprints = requestedIndexes ? { ...state.curriculumSavedFingerprints } : {};
    curricula.forEach((curriculum, index) => {
      if (!requestedIndexes || requestedIndexes.has(index)) {
        nextFingerprints[String(Number(curriculum.admissionYear))] = curriculumEditFingerprint(curriculum);
      }
    });
    state.curriculumSavedFingerprints = nextFingerprints;
  }

  function pendingCurriculumIsDirty(curriculumIndex) {
    const curriculum = pendingCurriculumItems()[Number(curriculumIndex)];
    if (!curriculum) return false;
    const savedFingerprint = state.curriculumSavedFingerprints[String(Number(curriculum.admissionYear))];
    return savedFingerprint === undefined || savedFingerprint !== curriculumEditFingerprint(curriculum);
  }

  function pendingCurriculumHasUnsavedChanges(currentOnly = false) {
    if (!state.pendingCurriculum) return false;
    preparePendingCurriculumForEditing();
    if (currentOnly) return pendingCurriculumIsDirty(state.curriculumPreviewIndex);
    return pendingCurriculumItems().some((_curriculum, index) => pendingCurriculumIsDirty(index));
  }

  function requestCurriculumLeave(action, { currentOnly = false } = {}) {
    if (!pendingCurriculumHasUnsavedChanges(currentOnly)) return Promise.resolve(action());
    curriculumLeaveConfirmAction = action;
    if (!curriculumLeaveDialog) {
      if (confirm("작업 중인 편제표가 저장되지 않았습니다. 그래도 나가시겠습니까?")) {
    ...
    if (curriculumLeaveDialog.open) curriculumLeaveDialog.close();
    curriculumLeaveDialog.showModal();
    requestAnimationFrame(() => curriculumLeaveDialog.querySelector("[data-confirm-curriculum-leave]")?.focus({ preventScroll: true }));
    return Promise.resolve(false);
  }
  ...
  function renderHeaderSchoolPicker() {
    ...
    options.innerHTML = !keyword
      ...
      ? schools.map((school) => {
        ...
        return `<button type="button" class="${isSelected ? "is-selected" : ""}" data-school-id="${escapeHtml(school.id)}"><small>${escapeHtml(school.region || "지역 정보 없음")} · ${years.length}개 입학년도</small><strong>${escapeHtml(school.name)}</strong></button>`;
      }).join("")
    ...
    const pendingSchool = state.schools.find((school) => school.id === state.schoolPickerPendingId) || null;
    if (listView) listView.hidden = Boolean(pendingSchool);
    if (yearView) yearView.hidden = !pendingSchool;
    if (pendingSchool && yearOptions) {
      const years = schoolAdmissionYears(pendingSchool);
      if (yearMeta) yearMeta.textContent = `${pendingSchool.region || "지역 정보 없음"} · ${years.length}개 입학년도`;
      if (yearName) yearName.textContent = pendingSchool.name;
      yearOptions.innerHTML = years.length
        ? years.map((year) => `<button type="button" data-school-connect-year="${year}">${year}년 입학생</button>`).join("")
        : "<em>등록된 편제표가 없습니다.</em>";
    }
  }

  function closeHeaderSchoolPicker() {
    const picker = document.querySelector(".header-school-picker");
  ...
  function openHeaderSchoolPicker(schoolId = "") {
    const picker = document.querySelector(".header-school-picker");
    const menu = picker?.querySelector("[data-school-menu]");
    if (!menu) return;
    state.schoolPickerPendingId = String(schoolId || "");
    renderHeaderSchoolPicker();
  ...
  function subjectUniversityButton(subject, kind, departmentId) {
    const isReflected = kind === "reflectedSubjects";
    const isExpanded = state.dialogDepartmentId === departmentId && state.dialogSubjectKind === kind && state.dialogSubjectName === subject.name;
    const universityCount = subject.universities.length.toLocaleString("ko-KR");
    const universityLabel = isReflected ? "반영 대학" : "권장 대학";
    return `<div class="major-subject-card ${isReflected ? "is-reflected" : "is-science"} ${isExpanded ? "is-expanded" : ""}">
      <button class="major-subject-guide" type="button" data-major-course="${escapeHtml(subject.name)}" aria-haspopup="dialog" aria-label="${escapeHtml(subject.name)} 과목 안내 열기"><span>${escapeHtml(subject.name)}</span>${isReflected ? reflectionStarMarkup() : ""}</button>
      <button class="major-subject-university-trigger" type="button" data-major-subject-universities data-department-id="${escapeHtml(departmentId)}" data-subject-kind="${escapeHtml(kind)}" data-subject-name="${escapeHtml(subject.name)}" aria-expanded="${isExpanded}" aria-label="${escapeHtml(subject.name)} ${universityLabel} ${universityCount}개 확인"><span>${universityCount}개 대학</span><i aria-hidden="true"></i></button>
    </div>`;
  }
  ...
  function bookUniversityRevealMarkup(department, books) {
    if (state.dialogDepartmentId !== department.id || state.dialogBookIndex < 0) return "";
    const book = books[state.dialogBookIndex];
    if (!book) return "";
    const universities = book.universities || [];
    return `<aside class="university-reveal major-book-university-reveal" aria-live="polite"><div><small>추천 대학</small><strong>${escapeHtml(book.title)}</strong><span>DB에 포함된 ${universities.length.toLocaleString("ko-KR")}개 대학</span></div><div class="university-list">${universities.map((university) => `<span>${escapeHtml(university)}</span>`).join("") || "<span>대학 정보 없음</span>"}</div></aside>`;
  }

  function departmentRecommendedBooksMarkup(department) {
    const entries = [...(department.recommendedBooks || [])].sort((a, b) => String(a.title || "").localeCompare(String(b.title || ""), "ko") || String(a.author || "").localeCompare(String(b.author || ""), "ko"));
    return `
      <section class="major-book-section">
        ...
        ${entries.length ? `<div class="major-book-table" role="list" aria-label="학과 권장 도서 목록">${entries.map((book, bookIndex) => {
          const isExpanded = state.dialogDepartmentId === department.id && state.dialogBookIndex === bookIndex;
          const universityCount = (book.universities || []).length;
          const universityButton = universityCount
            ? `<button class="major-book-university-trigger" type="button" data-major-book-universities data-department-id="${escapeHtml(department.id)}" data-book-index="${bookIndex}" aria-expanded="${isExpanded}" aria-label="${escapeHtml(book.title)} 추천 대학 ${universityCount.toLocaleString("ko-KR")}개 확인"><span>${universityCount.toLocaleString("ko-KR")}개 대학</span><i aria-hidden="true"></i></button>`
            : "";
          const mobileUniversities = universityCount ? recommendedBookUniversityBadgesMarkup(book.universities, "major-book-mobile-universities") : "";
          return `<article class="major-book-row ${universityCount ? "has-university" : ""} ${isExpanded ? "is-expanded" : ""}" role="listitem"><strong class="major-book-title">${escapeHtml(book.title)}</strong><span class="major-book-author">${escapeHtml(book.author || "저자 정보 없음")}</span><div class="major-book-university-cell ${universityCount ? "" : "is-empty"}"${universityCount ? "" : ' aria-hidden="true"'}>${universityButton}${mobileUniversities}</div></article>`;
        }).join("")}</div>${bookUniversityRevealMarkup(department, entries)}` : '<p class="major-book-empty">등록된 권장 도서가 없습니다.</p>'}
      </section>`;
  }
  ...
  function openDepartment(id, options = {}) {
    ...
    if (!options.keepBook) state.dialogBookIndex = -1;
    if (!options.keepSubject && !options.keepBook) {
    ...
    const relatedMarkup = majorSubjectGroupsMarkup(department.relatedSubjects, (subject) => reflectedNames.has(subject)
      ? `<button class="major-subject-card is-related is-related-reflected" type="button" data-major-course="${escapeHtml(subject)}" aria-haspopup="dialog" aria-label="${escapeHtml(subject)} 과목 안내 열기"><span>${escapeHtml(subject)}</span>${reflectionStarMarkup()}</button>`
      : `<button class="major-subject-card is-related" type="button" data-major-course="${escapeHtml(subject)}" aria-haspopup="dialog" aria-label="${escapeHtml(subject)} 과목 안내 열기"><span>${escapeHtml(subject)}</span></button>`);
    detailContent.innerHTML = `
      ...
      <section class="major-course-section is-related">
        <div class="major-course-title"><span>${icon("book")}</span><div><small>RELATED COURSES</small><h3>관련 과목</h3></div><em>${department.relatedSubjects.length}</em></div>
        ${department.reflectedSubjects.length ? reflectionMeaningNoteMarkup() : ""}
        ${relatedMarkup}
      </section>
      ${department.reflectedSubjects.length ? `<section class="major-course-section is-reflected"><div class="major-course-title"><span class="reflection-section-icon">${icon("solid-star")}</span><div><small>ADMISSION REFLECTION</small><h3>반영 과목 <b>중요</b></h3><p class="major-university-hint">대학버튼을 누르면 반영 대학을 확인할 수 있습니다.</p></div><em>${department.reflectedSubjects.length}</em></div>${subjectUniversityButtons(department.reflectedSubjects, "reflectedSubjects", department.id)}${universityRevealMarkup(department, "reflectedSubjects", state.dialogSubjectName)}</section>` : ""}
    ...
    if (courseNameColumn) {
      ...
      return `
        ...
        <section class="toolbar subject-toolbar" aria-label="과목 검색과 개설 교과 필터">
          ...
          <button class="school-course-toggle ${state.schoolOnlyCourses ? "is-active" : ""}" type="button" data-school-course-toggle aria-pressed="${state.schoolOnlyCourses}" title="${state.curriculum ? `${escapeHtml(state.selectedSchool?.name || "선택 학교")}의 개설 교과만 보기` : "학교를 선택하고 개설 교과를 확인합니다."}"><span class="school-course-toggle-title">${icon("check")}<strong>개설 교과</strong></span>${state.selectedSchool?.name ? `<small class="school-course-school-badge">${icon("school")}<b>${escapeHtml(state.selectedSchool.name)}</b></small>` : '<small class="school-course-school-badge is-empty">학교 선택 필요</small>'}</button>
        </section>
        ${resultsMarkup}`;
    }
  ...
  function openComparisonPicker() {
    ...
    detailContent.innerHTML = `${comparisonPickerMarkup()}${dialogFooterMarkup()}`;
    if (!detailDialog.open) detailDialog.showModal();
  ...
  function openComparisonResult() {
    ...
    detailContent.innerHTML = `${comparisonMarkup()}${dialogFooterMarkup()}`;
    if (!detailDialog.open) detailDialog.showModal();
    detailDialog.scrollTop = 0;
  }
  ...
  function fieldCommonSubjectsMarkup(subjects) {
    return majorSubjectGroupsMarkup(subjects, (subject) => `<span class="major-subject-card common-major-subject-card" title="${escapeHtml(subject.name)}"><strong>${escapeHtml(subject.name)}</strong><small class="common-major-coverage"><b>${subject.coverageCount}/${subject.totalCount}</b><span>개 학과</span></small></span>`, { className: "is-field-common", listClass: "major-subject-list common-major-subjects" });
  }
    ...
    return `${recommendStepHeading(2, "관심 학과 선택", "관심 학과를 최대 3개까지 선택할 수 있습니다. 다음 단계에서 공통 과목과 학과별 과목을 확인합니다.", `${selectedDepartments.length}/3 선택`)}
      <div class="recommend-department-stage">
        <section class="recommend-department-picker">
          ...
          <div class="recommend-department-list-head"><strong>학과 목록</strong><span>목록 안에서 스크롤</span></div>
        ...
        <aside class="recommend-major-selection ${selectedDepartments.length ? "has-selections" : "is-empty"}" style="--field-accent:${visual.accent}; --field-soft:${visual.soft}">
          ...
          <div class="recommend-major-selection-list">${selectedDepartments.length ? selectedDepartments.map((department, index) => `<button class="recommend-major-selection-item" type="button" data-recommend-department-choice="${escapeHtml(department.id)}" aria-pressed="true" aria-label="${escapeHtml(department.name)} 선택 해제"><b>${String(index + 1).padStart(2, "0")}</b><strong>${escapeHtml(department.name)}</strong><i aria-hidden="true">×</i></button>`).join("") : `<p>관심 학과를 하나 이상 선택해 주세요.</p>`}</div>
          ${selectedDepartments.length ? `<button class="recommend-selection-confirm" type="button" data-recommend-next>학과 선택 확인 바로가기 ${icon("arrow")}</button>` : ""}
  ...
  function recommendResultEntryMarkup(entry) {
    return `<button type="button" data-recommend-course="${escapeHtml(entry.name)}" aria-haspopup="dialog" aria-label="${escapeHtml(entry.name)} 과목 안내 열기"><strong>${escapeHtml(entry.name)}</strong><small>${escapeHtml(entry.detail || "과목 정보를 확인해 보세요.")}</small>${entry.count > 1 ? `<b>${entry.count}개 학과 공통</b>` : ""}</button>`;
  }
  ...
  const CURRICULUM_RESULT_TYPES = Object.freeze([
    { key: "completed", label: "수강 완료 과목", className: "is-completed" },
  ...
  function semesterCurriculumMarkup(gradeData, semesterData, semesterProgress, locked = false) {
    ...
    const optionsMarkup = semesterData.options.length
      ...
      : "";
    const electiveCourseCount = standalone.length + semesterData.options.reduce((sum, option) => sum + option.courses.length, 0);
    ...
    return `<section class="semester-curriculum-section ${locked ? "is-locked" : ""}" data-semester="${semesterData.semester}">
      ...
      <section class="common-course-block semester-subject-block"><header class="semester-subject-heading"><div><small>COMMON · SCHOOL DESIGNATED</small><h3>공통·학교 지정과목</h3></div><em>${semesterData.common.length}과목 자동 포함</em></header><div class="common-course-list">${commonMarkup}</div></section>
      <section class="semester-elective-block semester-subject-block"><header class="semester-subject-heading"><div><small>STUDENT ELECTIVES</small><h3>선택 과목</h3></div><em>${electiveCourseCount}과목</em></header>${optionArea}</section>
      ${locked ? '<button class="semester-lock-hit-area" type="button" data-semester-lock-info aria-label="2학기 선택 안내"></button>' : ""}
    </section>`;
  }
  ...
  function simulationSchoolOptionsMarkup(schools) {
    return schools.length
      ? schools.map((school, index) => {
        ...
        const years = schoolAdmissionYears(school);
        return `<button type="button" class="${selected ? "is-selected" : ""}" data-simulation-school-id="${escapeHtml(school.id)}"><span>${String(index + 1).padStart(2, "0")}</span><div><small>${escapeHtml(school.region || "지역 정보 없음")} · ${years.length}개 입학년도</small><strong>${escapeHtml(school.name)}</strong></div>${icon("arrow")}</button>`;
      }).join("")
  ...
  function simulationHistorySelectionMarkup() {
    ...
    return `<section class="simulation-history-stage">
      <header><div><h2>수강 완료 과목</h2><span>실제로 수강한 과목이 맞는지 확인하세요.</span></div><strong>${fixedCount}과목 자동 · ${historyProgress.selectedCount}과목 선택</strong></header>
  ...
  function simulationGradeProgressMarkup(progress) {
    const historyComplete = curriculumSelectionProgress(completedCurriculumGrades()).complete;
    const allSelectionsComplete = historyComplete && progress.complete;
    const gradeItems = progress.gradeProgress.map((gradeProgress) => {
      const accessible = allSelectionsComplete || gradeProgress.grade <= state.simulationMaxGradeStep || state.simulationResultUnlocked;
      const active = !state.simulationHistoryOpen && !state.simulationResultOpen && state.simulationGradeStep === gradeProgress.grade;
      const confirmed = allSelectionsComplete || gradeProgress.grade < state.simulationMaxGradeStep || state.simulationResultUnlocked;
    ...
    const resultAvailable = allSelectionsComplete;
    const resultCompleted = progress.complete && state.simulationResultUnlocked;
    return `<ol class="simulation-grade-progress" data-simulation-stage-start tabindex="-1" aria-label="학년별 과목 선택 진행 단계"><li class="is-history ${state.simulationHistoryOpen ? "is-active" : "is-complete"}"><button type="button" data-open-simulation-history aria-current="${state.simulationHistoryOpen ? "step" : "false"}"><span>${state.simulationHistoryOpen ? "01" : icon("check")}</span><strong>수강 완료 과목</strong><small>${completedCourseSubjects().length}과목</small></button></li>${gradeItems}<li class="is-result ${resultActive ? "is-active" : ""} ${resultCompleted ? "is-complete" : ""}"><button type="button" data-show-simulation-result ${resultAvailable ? "" : "disabled"} aria-current="${resultActive ? "step" : "false"}"><span>${resultCompleted ? icon("check") : String(progress.gradeProgress.length + 2).padStart(2, "0")}</span><strong>최종 결과</strong><small>${resultAvailable ? "확인 가능" : "선택 중"}</small></button></li></ol>`;
  }

  function focusSimulationStageStart() {
    requestAnimationFrame(() => {
      const stageStart = root.querySelector("[data-simulation-stage-start]");
      if (!stageStart) return;
      stageStart.focus({ preventScroll: true });
      const reduceMotion = window.matchMedia?.("(prefers-reduced-motion: reduce)").matches;
      stageStart.scrollIntoView({ behavior: reduceMotion ? "auto" : "smooth", block: "start" });
    });
  }
  ...
  function renderSimulation() {
    if (!state.selectedSchool || !state.curriculum || !SUPPORTED_ADMISSION_YEARS.includes(Number(state.selectedAdmissionYear || state.curriculum?.admissionYear))) {
      ...
      root.innerHTML = `
        ${renderNotices()}
        ${pageHead("모의 수강신청", "학교와 입학년도를 선택한 뒤, 수강 완료 과목부터 앞으로 선택할 과목까지 차례로 구성합니다.", 0, "연동 옵션")}
    ...
    if (state.simulationHistoryOpen) {
      ...
      root.innerHTML = `
        ${renderNotices()}
        ${pageHead("모의 수강신청", `${state.selectedSchool.name} ${state.curriculum.admissionYear || ""}년 입학생 · 먼저 실제로 수강한 과목을 확인합니다.`, completedCount, "수강 완료 과목")}
    ...
    root.innerHTML = `
      ...
      <section class="simulation-overview">
        ...
        <button class="text-action" type="button" data-clear-school-simulation ${progress.selectedCount + completedCount ? "" : "disabled"}>선택 초기화</button>
      </section>
  ...
  function sheetMatrixWithMergedValues(workbook, sheetName) {
    ...
    try {
      rangeStart = sheet?.["!ref"] ? window.XLSX.utils.decode_range(sheet["!ref"]).s : rangeStart;
    } catch (_error) {
      rangeStart = { r: 0, c: 0 };
    }
    // sheet_to_json 배열은 !ref의 시작 셀을 0,0으로 삼지만 병합 좌표는 원본 시트 좌표를 유지한다.
    const merges = (Array.isArray(sheet?.["!merges"]) ? sheet["!merges"] : [])
      .map((merge) => ({
        s: { r: merge.s.r - rangeStart.r, c: merge.s.c - rangeStart.c },
        e: { r: merge.e.r - rangeStart.r, c: merge.e.c - rangeStart.c }
      }))
      .filter((merge) => merge.e.r >= 0 && merge.e.c >= 0 && merge.s.r < raw.length);
    merges.forEach((merge) => {
      const value = raw[merge.s.r]?.[merge.s.c];
      for (let row = Math.max(0, merge.s.r); row <= merge.e.r; row += 1) {
        if (!filled[row]) filled[row] = [];
        for (let column = Math.max(0, merge.s.c); column <= merge.e.c; column += 1) {
  ...
  function uploadedCurriculumSectionKind(value) {
    const text = normalizedKey(value);
    if (!text || /소계|합계/u.test(text)) return "";
    if (text.includes("선택")) return "selection";
    if (text.includes("지정") || text.includes("공통") || text.includes("필수")) return "fixed";
    return "";
  }
  ...
  function fullYearCurriculumRowHasPlacement(source, row, semesterColumns) {
    return semesterColumns.some(({ column }) => {
      const value = source.filled[row]?.[column];
      return isCoursePlacementValue(value) || parseChoiceCount(value) > 0;
    });
  }

  function fullYearCurriculumColumnEvidence(source, column, dataStartRow, lastCourseRow, semesterColumns) {
    ...
    const evidence = {
      ...
      courseCount: 0,
      courseOnlyCount: 0,
      coursePlacementCount: 0,
      courseOnlyPlacementCount: 0,
      groupCount: 0,
      groupOnlyCount: 0,
      groupPlacementCount: 0,
      groupOnlyPlacementCount: 0,
      typeCount: 0,
      typePlacementCount: 0,
      sectionCount: 0,
      sectionPlacementCount: 0
    };
    for (let row = dataStartRow; row <= lastCourseRow; row += 1) {
      const value = compactText(source.filled[row]?.[column]);
      if (!value || /소계|합계/u.test(value)) continue;
      evidence.valueCount += 1;
      const hasPlacement = fullYearCurriculumRowHasPlacement(source, row, semesterColumns);
      ...
      const isType = knownTypeKeys.has(normalizedKey(uploadedCourseType(value)));
      const isSection = Boolean(uploadedCurriculumSectionKind(value));
      if (isCourse) evidence.courseCount += 1;
      if (isCourse && !isGroup) evidence.courseOnlyCount += 1;
      if (isCourse && hasPlacement) evidence.coursePlacementCount += 1;
      if (isCourse && !isGroup && hasPlacement) evidence.courseOnlyPlacementCount += 1;
      if (isGroup) evidence.groupCount += 1;
      if (isGroup && !isCourse) evidence.groupOnlyCount += 1;
      if (isGroup && hasPlacement) evidence.groupPlacementCount += 1;
      if (isGroup && !isCourse && hasPlacement) evidence.groupOnlyPlacementCount += 1;
      if (isType) evidence.typeCount += 1;
      if (isType && hasPlacement) evidence.typePlacementCount += 1;
      if (isSection) evidence.sectionCount += 1;
      if (isSection && hasPlacement) evidence.sectionPlacementCount += 1;
    }
    return evidence;
  }

  function fullYearCurriculumRoleRelationCount(source, courseColumn, roleColumn, role, dataStartRow, lastCourseRow, semesterColumns) {
    const referenceIndex = recommendationCourseIndex();
    let count = 0;
    for (let row = dataStartRow; row <= lastCourseRow; row += 1) {
      if (!fullYearCurriculumRowHasPlacement(source, row, semesterColumns)) continue;
      const courseName = compactText(source.raw[row]?.[courseColumn] || source.filled[row]?.[courseColumn]);
      if (!courseName || /소계|합계/u.test(courseName)) continue;
      const roleValue = source.filled[row]?.[roleColumn];
      if (role === "category") {
        const reference = curriculumCourseReference(courseName, referenceIndex);
        if (reference && normalizedCourseName(normalizeCourseGroup(roleValue)) === normalizedCourseName(reference.category)) count += 1;
      } else if (role === "type" && ["공통", "일반", "진로", "융합", "전문"].includes(uploadedCourseType(roleValue))) count += 1;
      else if (role === "section" && uploadedCurriculumSectionKind(roleValue)) count += 1;
    }
    return count;
  }
  ...
  function compareFullYearColumnRole(first, second, role) {
    ...
    if (role === "course") {
      return second.evidence.courseOnlyPlacementCount - first.evidence.courseOnlyPlacementCount
        || (second.relationshipCount || 0) - (first.relationshipCount || 0)
        || second.evidence.courseOnlyCount - first.evidence.courseOnlyCount
        || second.evidence.coursePlacementCount - first.evidence.coursePlacementCount
    ...
    if (role === "category") {
      return (second.relationshipCount || 0) - (first.relationshipCount || 0)
        || second.evidence.groupOnlyPlacementCount - first.evidence.groupOnlyPlacementCount
        || second.evidence.groupOnlyCount - first.evidence.groupOnlyCount
        || second.evidence.groupPlacementCount - first.evidence.groupPlacementCount
    ...
    if (role === "section") {
      return (second.relationshipCount || 0) - (first.relationshipCount || 0)
        || second.evidence.sectionPlacementCount - first.evidence.sectionPlacementCount
        || second.evidence.sectionCount - first.evidence.sectionCount
        || ratio(second, "sectionCount") - ratio(first, "sectionCount")
        || (first.distance || 0) - (second.distance || 0);
    }
    return (second.relationshipCount || 0) - (first.relationshipCount || 0)
      || second.evidence.typePlacementCount - first.evidence.typePlacementCount
      || second.evidence.typeCount - first.evidence.typeCount
  ...
  function parseSchoolFullYearCurriculumWorkbook(file, workbook) {
    ...
    const dataEndRow = source.filled.findIndex((row, index) => index >= dataStartRow && row.some((value) => {
      const key = normalizedKey(value);
      return key.includes(normalizedKey("교과 이수 학점 소계")) || /^교과(?:이수학점)?(?:소계|합계)$/u.test(key);
    }));
    const lastCourseRow = dataEndRow < 0 ? source.raw.length - 1 : dataEndRow - 1;
    const firstSemesterColumn = Math.min(...semesterColumns.map(({ column }) => column));
    const roleCandidateIndices = [...new Set([
      ...Array.from({ length: Math.max(0, firstSemesterColumn) }, (_value, index) => index),
      ...courseCandidates
    ])];
    const candidateEvidence = roleCandidateIndices.map((index) => ({
      index,
      headerMatch: courseCandidates.includes(index),
      evidence: fullYearCurriculumColumnEvidence(source, index, dataStartRow, lastCourseRow, semesterColumns)
    }));
    candidateEvidence.forEach((candidate) => {
      candidate.relationshipCount = ["category", "type", "section"].reduce((sum, role) => {
        const neighboringSupport = candidateEvidence
          .filter((other) => other.index !== candidate.index)
          .map((other) => fullYearCurriculumRoleRelationCount(
            source,
            candidate.index,
            other.index,
            role,
            ...
            lastCourseRow,
            semesterColumns
          ) / Math.max(1, Math.abs(candidate.index - other.index)))
          .sort((first, second) => second - first)[0] || 0;
        return sum + neighboringSupport;
      }, 0);
    });
    ...
    // 기존 머리글 후보를 유지하면서, 실제 학기 배치 행에서 과목 DB 일치 근거가 있는 열만 우선한다.
    const evidencedCourseIndex = candidateEvidence
      .filter((candidate) => candidate.headerMatch && candidate.evidence.courseOnlyPlacementCount > 0)
    ...
    if (categoryIndex < 0) {
      categoryIndex = candidateEvidence
        ...
        .map((candidate) => ({
          ...candidate,
          distance: Math.abs(candidate.index - courseIndex),
          relationshipCount: fullYearCurriculumRoleRelationCount(source, courseIndex, candidate.index, "category", dataStartRow, lastCourseRow, semesterColumns)
        }))
    ...
    if ([courseIndex, categoryIndex].includes(typeIndex)) typeIndex = -1;
    if (typeIndex < 0) {
      typeIndex = candidateEvidence
        ...
        .map((candidate) => ({
          ...candidate,
          distance: Math.abs(candidate.index - courseIndex),
          relationshipCount: fullYearCurriculumRoleRelationCount(source, courseIndex, candidate.index, "type", dataStartRow, lastCourseRow, semesterColumns)
        }))
        .sort((first, second) => compareFullYearColumnRole(first, second, "type"))[0]?.index ?? -1;
    }
    let sectionIndex = headerKeys.findIndex((key) => ["구분", "편성구분", "교육과정구분", "이수구분"].map(normalizedKey).includes(key));
    if ([courseIndex, categoryIndex, typeIndex].includes(sectionIndex)) sectionIndex = -1;
    if (sectionIndex < 0) {
      sectionIndex = candidateEvidence
        .filter((candidate) => ![courseIndex, categoryIndex, typeIndex].includes(candidate.index) && candidate.evidence.sectionCount > 0)
        .map((candidate) => ({
          ...candidate,
          distance: Math.abs(candidate.index - courseIndex),
          relationshipCount: fullYearCurriculumRoleRelationCount(source, courseIndex, candidate.index, "section", dataStartRow, lastCourseRow, semesterColumns)
        }))
        .sort((first, second) => compareFullYearColumnRole(first, second, "section"))[0]?.index ?? -1;
    }
    ...
    semesterColumns.forEach(({ column, grade, semester }) => {
      ...
      for (let row = dataStartRow; row <= lastCourseRow; row += 1) {
        const sectionKind = sectionIndex < 0 ? "" : uploadedCurriculumSectionKind(source.filled[row]?.[sectionIndex]);
        const courseName = compactText(source.raw[row]?.[courseIndex]);
        const isIndividualPlacement = courseName
          && isCoursePlacementValue(source.raw[row]?.[column])
          && !rowIsInChoiceGroup(row);
        if (isIndividualPlacement) {
          if (sectionKind === "selection") target.electives.push(courseName);
          else target.common.push(courseName);
  ...
  let hancomParserModulePromise = null;

  function htmlCurriculumWorkbook(html, fileName, expectedAdmissionYear = 0) {
    if (typeof DOMParser !== "function") throw new Error("한글 문서의 표 구조를 읽을 수 없는 브라우저입니다.");
    const parsedDocument = new DOMParser().parseFromString(String(html || ""), "text/html");
    const rows = [];
    const merges = [];
    const cellText = (cell) => {
      const paragraphs = [...cell.querySelectorAll("p")].map((paragraph) => compactText(paragraph.textContent)).filter(Boolean);
    ...
    parsedDocument.body.querySelectorAll("p, h1, h2, h3, h4, h5, h6, table").forEach((element) => {
      if (element.tagName !== "TABLE") {
        if (!element.closest("table")) addParagraph(element);
        return;
      }
      const tableStartRow = rows.length;
      const occupied = new Set();
      [...element.rows].forEach((tableRow, rowIndex) => {
        const targetRow = rows[tableStartRow + rowIndex] || [];
        let columnIndex = 0;
        [...tableRow.cells].forEach((cell) => {
          ...
          targetRow[columnIndex] = cellText(cell);
          if (rowSpan > 1 || columnSpan > 1) {
            merges.push({
              s: { r: tableStartRow + rowIndex, c: columnIndex },
              e: { r: tableStartRow + rowIndex + rowSpan - 1, c: columnIndex + columnSpan - 1 }
            });
          }
        ...
        rows[tableStartRow + rowIndex] = targetRow;
      });
      rows.push([]);
    });
    if (expectedAdmissionYear && !rows.flat().some((value) => new RegExp(`${expectedAdmissionYear}학년도`).test(compactText(value)))) {
      rows.unshift([`${expectedAdmissionYear}학년도 신입생 3개년 교육과정 편제표`]);
      merges.forEach((merge) => {
        merge.s.r += 1;
        merge.e.r += 1;
      });
    }
    if (!rows.some((row) => row.some((value) => compactText(value)))) throw new Error("한글 문서에서 편제표 내용을 찾지 못했습니다.");
    const sheet = typeof window.XLSX?.utils?.aoa_to_sheet === "function"
      ? window.XLSX.utils.aoa_to_sheet(rows)
      : { _matrix: rows };
    sheet["!merges"] = merges;
    return { SheetNames: ["한글 편제표"], Sheets: { "한글 편제표": sheet }, Props: { Title: fileName } };
  }

  async function hancomCurriculumWorkbook(file, expectedAdmissionYear = 0) {
    try {
      hancomParserModulePromise ||= import("https://cdn.jsdelivr.net/npm/@ssabrojs/hwpxjs@0.4.0/dist/browser/hwpxjs.browser.mjs");
      const hancomParser = await hancomParserModulePromise;
      const originalBuffer = await file.arrayBuffer();
      const extension = String(file.name || "").split(".").pop().toLocaleLowerCase("en");
      let hwpxBuffer = originalBuffer;
      if (extension === "hwp") {
        const converted = await hancomParser.hwpToHwpx(new Uint8Array(originalBuffer), { title: file.name, creator: "선택 과목 안내 플랫폼" });
        hwpxBuffer = converted instanceof ArrayBuffer
          ? converted
          : converted.buffer.slice(converted.byteOffset, converted.byteOffset + converted.byteLength);
      }
      const reader = new hancomParser.default();
      await reader.loadFromArrayBuffer(hwpxBuffer);
      const html = await reader.extractHtml({ renderImages: false, renderTables: true, renderStyles: false, tableHeaderFirstRow: false });
      return htmlCurriculumWorkbook(html, file.name, expectedAdmissionYear);
    } catch (error) {
      console.error("한글 편제표 분석 실패:", error);
      throw new Error("한글 파일을 분석하지 못했습니다. 암호·배포용 문서가 아닌 HWP 또는 HWPX 파일인지 확인해 주세요.");
    }
  }

  function curriculumFileWithAdmissionYear(file, expectedAdmissionYear) {
    if (!expectedAdmissionYear || new RegExp(`${expectedAdmissionYear}학년도.*(?:신입생|입학생)`).test(compactText(file.name))) return file;
    return {
      name: `${expectedAdmissionYear}학년도 신입생_${file.name}`,
      arrayBuffer: () => file.arrayBuffer()
    };
  }

  function validateParsedAdmissionYear(parsed, expectedAdmissionYear, originalFileName) {
    if (!expectedAdmissionYear || !parsed) return parsed;
    const curricula = Array.isArray(parsed.curricula) ? parsed.curricula : [parsed];
    const parsedYears = [...new Set(curricula.map((curriculum) => Number(curriculum?.admissionYear)).filter(Boolean))];
    if (parsedYears.some((year) => year !== Number(expectedAdmissionYear))) {
      throw new Error(`${originalFileName}은 ${parsedYears.join("·")}학년도 자료로 확인됩니다. ${expectedAdmissionYear}학년도 파일 선택란에 맞는 자료를 올려 주세요.`);
    }
    curricula.forEach((curriculum) => {
      curriculum.admissionYear = Number(expectedAdmissionYear);
      curriculum.sourceAdmissionYear = Number(expectedAdmissionYear);
      curriculum.fileName = originalFileName;
    });
    parsed.fileName = originalFileName;
    return parsed;
  }

  async function parseCurriculumFile(file, { admissionYear: expectedAdmissionYear = 0 } = {}) {
    if (!window.XLSX) throw new Error("엑셀 도구를 불러오지 못했습니다. 인터넷 연결을 확인해 주세요.");
    const extension = String(file.name || "").split(".").pop().toLocaleLowerCase("en");
    if (!["xlsx", "xls", "hwp", "hwpx"].includes(extension)) throw new Error("엑셀(.xlsx, .xls) 또는 한글(.hwp, .hwpx) 파일만 업로드할 수 있습니다.");
    const sourceFile = curriculumFileWithAdmissionYear(file, Number(expectedAdmissionYear));
    const workbook = ["hwp", "hwpx"].includes(extension)
      ? await hancomCurriculumWorkbook(file, Number(expectedAdmissionYear))
      : window.XLSX.read(await file.arrayBuffer(), { type: "array", cellDates: true });
    const fullYearCurriculum = parseSchoolFullYearCurriculumWorkbook(sourceFile, workbook);
    if (fullYearCurriculum) return validateParsedAdmissionYear(fullYearCurriculum, expectedAdmissionYear, file.name);
    const flexibleCurriculum = parseFlexibleFlatCurriculumWorkbook(sourceFile, workbook);
    if (flexibleCurriculum) return validateParsedAdmissionYear(flexibleCurriculum, expectedAdmissionYear, file.name);
    ...
    return validateParsedAdmissionYear({ version: 8, sourceFormat: "freshman-three-year-standard", sourceAcademicYear: currentAcademicYear(), sourceAdmissionYear: admissionYear, fileName: file.name, schoolName, region, admissionYear, grades, courseCount, unlistedCourseCount, uploadedAt: new Date().toISOString() }, expectedAdmissionYear, file.name);
  }
  ...
  function linkedSchoolForCurriculum(curriculum) {
    const schoolName = normalizedKey(curriculum?.schoolName);
    const region = normalizedKey(curriculum?.region);
    if (!schoolName || !region) return null;
    return state.schools.find((school) => normalizedKey(school.name) === schoolName && normalizedKey(school.region) === region) || null;
  }

  function curriculumCopySources(curricula, targetIndex) {
    const target = curricula[targetIndex];
    if (!target) return [];
    const targetYear = Number(target.admissionYear);
    const sources = [];
    const includedYears = new Set();
    curricula.forEach((curriculum, curriculumIndex) => {
      const admissionYear = Number(curriculum.admissionYear);
      if (curriculumIndex === targetIndex || admissionYear === targetYear || !curriculum.courseCount) return;
      sources.push({ value: `pending:${curriculumIndex}`, admissionYear, label: `${admissionYear}년 입학생 · 현재 작성 중` });
      includedYears.add(admissionYear);
    });
    const school = linkedSchoolForCurriculum(target);
    (school?.admissionYears || []).forEach((admissionYearValue) => {
      const admissionYear = Number(admissionYearValue);
      if (admissionYear === targetYear || includedYears.has(admissionYear)) return;
      sources.push({ value: `published:${admissionYear}`, admissionYear, label: `${admissionYear}년 입학생 · 저장됨` });
      includedYears.add(admissionYear);
    });
    return sources.sort((a, b) => b.admissionYear - a.admissionYear);
  }

  function copyCurriculumStructure(target, source) {
    if (!target || !source || !Array.isArray(source.grades)) return false;
    target.grades = JSON.parse(JSON.stringify(source.grades));
    target.courseMetadata = JSON.parse(JSON.stringify(source.courseMetadata || {}));
    target.copiedFromAdmissionYear = Number(source.admissionYear) || null;
  ...
  function createStoredCurriculumImport(sourceCurriculum = state.curriculum, sourceSchool = state.selectedSchool) {
    if (!sourceCurriculum || !sourceSchool) return null;
    const curriculum = JSON.parse(JSON.stringify(sourceCurriculum));
    ...
    curriculum.fileName = `${sourceSchool.name} ${curriculum.admissionYear}학년도 편제표`;
    curriculum.schoolId = sourceSchool.id;
    curriculum.schoolName = sourceSchool.name;
    curriculum.region = sourceSchool.region || curriculum.region || "";
    curriculum.parseWarnings = ["현재 등록된 편제표를 불러왔습니다. 내용을 확인하고 ‘편제표 수정’을 누르면 기존 자료가 교체됩니다."];
    return curriculum;
  }
  ...
  async function closeCurriculumPreview() {
  ...
  function returnToTeacherCurriculumUpload(identity) {
    state.pendingCurriculum = null;
    state.curriculumPreviewIndex = 0;
    state.curriculumPreviewGradeIndex = 0;
    state.curriculumYearPickerOpen = false;
    ...
    renderAdmin();
    requestAnimationFrame(() => root.querySelector("[data-upload-year-select]")?.focus({ preventScroll: true }));
  }
  ...
  function combineFreshmanCurriculumImports(files, parsedImports) {
    ...
    return {
      ...
      parseWarnings: [`${sorted.map((curriculum) => curriculum.admissionYear).join("·")}학년도 입학생의 3개년 편제표를 각각 읽었습니다.`],
      academicYear: currentAcademicYear(),
  ...
  async function reviewCurriculumFiles(fileList) {
    const fileEntries = Array.from(fileList || []).map((entry) => entry?.file
      ? { file: entry.file, admissionYear: Number(entry.admissionYear) || 0 }
      : { file: entry, admissionYear: 0 });
    const files = fileEntries.map((entry) => entry.file);
    if (!files.length || files.length > SUPPORTED_ADMISSION_YEARS.length) {
      showCurriculumAlert("입학년도별 편제표를 선택해 주세요", `등록할 ${SUPPORTED_ADMISSION_YEARS.join("·")}학년도 신입생 편제표를 입학년도마다 한 개씩 선택해 주세요.`);
      resetCurriculumDraftState();
      await releaseTeacherCurriculumAccess();
      renderAdmin();
      return;
    ...
    state.curriculumCoursePicker = null;
    state.curriculumSavedFingerprints = {};
    state.curriculumImportMessage = `신입생 편제표 ${files.length}개를 입학년도별로 검증하고 있습니다.`;
    renderAdmin();
    try {
      const parsedImports = await Promise.all(fileEntries.map(({ file, admissionYear }) => parseCurriculumFile(file, { admissionYear })));
      state.pendingCurriculum = applyPendingCurriculumIdentity(combineFreshmanCurriculumImports(files, parsedImports));
      resetCurriculumDraftState({ clearIdentity: false });
      preparePendingCurriculumForEditing();
      state.curriculumSavedFingerprints = {};
      state.curriculumImportMessage = `검증 완료: 신입생 편제표 ${files.length}개에서 ${state.pendingCurriculum.courseCount.toLocaleString("ko-KR")}개 교과와 학년별 옵션을 확인했습니다.`;
      showToast(`신입생 편제표 ${files.length}개의 검증이 완료되었습니다.`);
    } catch (error) {
  ...
  function curriculumCoursePickerCatalog() {
    ...
    (picker?.customCourses || []).forEach((course) => {
  ...
  function curriculumCoursePickerMarkup() {
    ...
    const customCourseMarkup = picker.lane === "standalone" || picker.lane === "option"
    ...
    return `<div class="curriculum-course-picker-overlay" data-curriculum-course-picker-overlay>
      <section class="curriculum-course-picker ${customCourseMarkup ? "has-custom-course-entry" : ""}" role="dialog" aria-modal="true" aria-labelledby="curriculum-course-picker-title">
  ...
  function curriculumEditorLaneMarkup({ curriculumIndex, grade, semester, lane, optionIndex = "", title, courses, addLabel = "과목 추가", emptyLabel = "과목을 추가하세요." }) {
    ...
    return `<section class="curriculum-editor-lane ${lane === "option" ? "is-option" : ""}">
      ...
      <div class="curriculum-course-dropzone ${courses.length ? "" : "is-empty"}" data-curriculum-course-dropzone ${attributes}>
        ${courses.length ? courses.map((course, courseIndex) => curriculumEditorCourseMarkup(course, courseIndex, attributes, courseMetadata)).join("") : `<p>${escapeHtml(emptyLabel)}</p>`}
      </div>
    </section>`;
  }

  function curriculumOptionEditorMarkup(curriculumIndex, grade, semester, option, optionIndex) {
    const attributes = curriculumEditorDataAttributes(curriculumIndex, grade, semester, "option", optionIndex);
    return `<article class="curriculum-option-editor">
      ...
      ${curriculumEditorLaneMarkup({ curriculumIndex, grade, semester, lane: "option", optionIndex, courses: option.courses, addLabel: "선택 과목 추가" })}
    </article>`;
  }
  ...
  function curriculumEditorSemesterMarkup(curriculum, curriculumIndex, grade, semester) {
    const commonLane = curriculumEditorLaneMarkup({
      ...
      addLabel: "공통·학교 지정과목 추가",
      emptyLabel: "공통·학교 지정과목을 추가하세요."
    });
    const standaloneLane = curriculumEditorLaneMarkup({
      curriculumIndex,
      grade: grade.grade,
      semester: semester.semester,
      lane: "standalone",
      ...
      addLabel: "선택 과목 추가",
      emptyLabel: "선택 과목을 추가하세요."
    });
    const optionMarkup = semester.options.map((option, optionIndex) => curriculumOptionEditorMarkup(
      curriculumIndex,
      grade.grade,
      semester.semester,
      option,
    ...
    return `<article class="curriculum-semester-editor" data-curriculum-semester-editor data-curriculum-index="${curriculumIndex}" data-curriculum-grade="${grade.grade}" data-curriculum-semester="${semester.semester}">
      <header><div><small>${grade.grade}학년</small><h5>${semester.semester}학기</h5></div><span data-curriculum-semester-summary>공통·학교 지정 ${semester.common.length} · 선택 옵션 ${semester.options.length}</span></header>
      <section class="curriculum-common-section">
        <header class="curriculum-elective-heading curriculum-common-heading"><div><small>COMMON · SCHOOL DESIGNATED</small><strong>공통·학교 지정 과목</strong></div><span data-curriculum-common-summary>${semester.common.length}과목</span></header>
        ${commonLane}
      </section>
      <section class="curriculum-elective-section">
        <header class="curriculum-elective-heading"><div><small>STUDENT ELECTIVES</small><strong>선택 과목</strong></div><span data-curriculum-elective-summary>개별 ${semester.standalone.length}과목 · 옵션 ${semester.options.length}개</span></header>
        ${standaloneLane}
  ...
  function refreshCurriculumEditorSummaries() {
    ...
    curricula.forEach((curriculum, curriculumIndex) => {
      ...
      const published = linkedSchoolForCurriculum(curriculum)?.admissionYears?.map(Number).includes(Number(curriculum.admissionYear));
      const editStatus = curriculum.courseCount ? `${gradeLabel}학년 편제 작성됨` : "아직 작성하지 않음";
      const yearOptionSummary = root.querySelector(`[data-curriculum-year-option="${curriculumIndex}"] small`);
      const statusText = `${editStatus}${published ? " · 등록됨" : ""}`;
      if (yearOptionSummary) yearOptionSummary.textContent = statusText;
      if (curriculumIndex === state.curriculumPreviewIndex) {
        const currentYearSummary = root.querySelector("[data-curriculum-year-current-summary]");
        if (currentYearSummary) currentYearSummary.textContent = statusText;
      }
    });
  ...
  function refreshCurriculumCoursePickerInPlace({ focusSearch = false, focusCustom = false, resultsOnly = false, customOnly = false } = {}) {
    ...
    const selectors = customOnly
      ? [".curriculum-picker-custom-action"]
      : resultsOnly
        ? [".curriculum-picker-results"]
        : [".curriculum-picker-categories", ".curriculum-picker-results", ".curriculum-picker-custom-action", "footer"];
    ...
    if (focusCustom) current.querySelector("[data-curriculum-custom-course-form] input[name='customCourse']")?.focus({ preventScroll: true });
    return true;
  }
  ...
  function refreshCurriculumPreviewSelectionInPlace({ pageChanged = false } = {}) {
    ...
    const selectors = pageChanged
      ? [".curriculum-year-workspace-selector", ".curriculum-region-field", ".curriculum-editor-cohort", ".preview-help"]
  ...
  function validatePendingCurriculumEdits(curriculumIndexes = null) {
    const allCurricula = preparePendingCurriculumForEditing();
    const requestedIndexes = Array.isArray(curriculumIndexes) ? new Set(curriculumIndexes.map(Number)) : null;
    const curricula = requestedIndexes ? allCurricula.filter((_curriculum, index) => requestedIndexes.has(index)) : allCurricula;
    const errors = [];
    const years = new Set();
    if (!curricula.length) errors.push("등록할 입학년도 편제표를 찾지 못했습니다.");
    curricula.forEach((curriculum) => {
      if (!schoolNamePrefix(curriculum.schoolName) || !compactText(curriculum.schoolName).endsWith("고등학교")) errors.push("OO 고등학교 형식으로 작성해 주세요");
      ...
      if (allCurricula.filter((entry) => Number(entry.admissionYear) === Number(curriculum.admissionYear)).length > 1) errors.push(`${curriculum.admissionYear}년 입학생 편제표가 편집 화면에 중복되어 있습니다.`);
      if (years.has(Number(curriculum.admissionYear))) errors.push("입학년도가 서로 중복됩니다.");
      years.add(Number(curriculum.admissionYear));
      if (!curriculum.courseCount) errors.push(`${curriculum.admissionYear || "해당"}년 입학생 편제표에 과목이 없습니다. 상단의 입학년도 선택창을 확인해 주세요.`);
      curriculum.grades.forEach((grade) => grade.semesters.forEach((semester) => semester.options.forEach((option) => {
        ...
        if (!option.courses.length) errors.push(`${curriculum.admissionYear}년 입학생 · ${grade.grade}학년 ${semester.semester}학기 · ${option.label}에 과목이 없습니다. 사용하지 않는 옵션이면 ‘옵션 삭제’를 눌러 주세요.`);
        if (!Number.isInteger(Number(option.choose)) || Number(option.choose) < 1 || Number(option.choose) > option.courses.length) {
          errors.push(`${curriculum.admissionYear}년 입학생 · ${grade.grade}학년 ${semester.semester}학기 · ${option.label}의 선택 수는 1~${option.courses.length || 1} 사이여야 합니다.`);
        }
      })));
    });
    if (errors.length) throw new Error([...new Set(errors)].slice(0, 5).map((error) => `• ${error}`).join("\n"));
    return curricula;
  }
  ...
  function curriculumPreviewMarkup() {
    ...
    const linkedSchool = linkedSchoolForCurriculum(activeCurriculum);
    const publishedAdmissionYears = new Set((linkedSchool?.admissionYears || []).map(Number));
    const copySources = curriculumCopySources(curricula, state.curriculumPreviewIndex);
    ...
    const publishLabel = state.curriculumBusy ? "DB에 저장 중"
      : isAdminEdit ? "편제표 수정"
        : "편제표 등록";
    const yearOptions = curricula.map((curriculum, curriculumIndex) => {
      const gradeLabel = curriculum.grades.map((grade) => grade.grade).join("·");
      const editStatus = curriculum.courseCount ? `${gradeLabel}학년 편제 작성됨` : "아직 작성하지 않음";
      const publishStatus = publishedAdmissionYears.has(Number(curriculum.admissionYear)) ? " · 등록됨" : "";
      return `<button type="button" class="${curriculumIndex === state.curriculumPreviewIndex ? "is-selected" : ""}" data-curriculum-year-option="${curriculumIndex}" role="option" aria-selected="${curriculumIndex === state.curriculumPreviewIndex}"><span><strong>${curriculum.admissionYear}학년도 신입생</strong><small>${editStatus}${publishStatus}</small></span>${curriculumIndex === state.curriculumPreviewIndex ? icon("check") : ""}</button>`;
    }).join("");
    const activeGradeLabel = activeCurriculum.grades.map((grade) => grade.grade).join("·");
    const activeEditStatus = activeCurriculum.courseCount ? `${activeGradeLabel}학년 편제 작성됨` : "아직 작성하지 않음";
    const activePublishStatus = publishedAdmissionYears.has(Number(activeCurriculum.admissionYear)) ? " · 등록됨" : "";
  ...
  function schoolAuthDialogMarkup() {
    ...
    const authSchoolName = completeSchoolName(state.schoolAuthSchoolName);
    const existingUploadSchool = state.schools.find((school) => (
      normalizedKey(school.name) === normalizedKey(authSchoolName)
      && (!state.schoolAuthRegion || school.region === state.schoolAuthRegion)
    ));
    const linkedUploadYears = new Set((existingUploadSchool?.admissionYears || []).map(Number));
    const uploadDraftDate = state.uploadCurriculumDraft?.updatedAt ? new Date(state.uploadCurriculumDraft.updatedAt) : null;
    const uploadDraftStatus = uploadDraftDate && !Number.isNaN(uploadDraftDate.getTime())
    ...
    const uploadYearSlots = SUPPORTED_ADMISSION_YEARS.map((admissionYear) => {
      ...
      <header><div><small>${admissionYear} ADMISSION</small><strong>${admissionYear}학년도 신입생</strong></div></header>
      ${isLinked ? `<div class="curriculum-upload-existing"><span>${icon("check")}</span><strong>업로드 완료</strong><button type="button" data-edit-uploaded-curriculum="${admissionYear}">${icon("pen")} 편제표 수정</button></div>` : ""}
      <label class="school-auth-file"><span>${isLinked ? "새 파일로 교체" : "편제표 파일 선택"}</span><input type="file" accept=".xlsx,.xls,.hwp,.hwpx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel,application/x-hwp,application/haansofthwp,application/vnd.hancom.hwpx" data-auth-curriculum-file data-upload-admission-year="${admissionYear}"><small>엑셀(.xlsx, .xls) 또는 한글(.hwp, .hwpx) 파일 1개</small><b data-upload-file-name>선택된 파일 없음</b></label>
    </article>`;
    }).join("");
    const teacherUploadForm = `<form class="school-auth-form curriculum-year-upload-form" data-teacher-upload-form>
      ${uploadDraftResume}
      <section class="curriculum-upload-year-picker"><div><span>신입생 입학년도 선택</span><small>등록할 입학년도를 누르세요.</small></div><div role="group" aria-label="업로드할 신입생 입학년도">${uploadYearNavigation}</div></section>
      <div class="curriculum-year-upload-list">${uploadYearSlots}</div>
      <button class="primary-action" type="submit" data-upload-year-submit ${selectedUploadYear ? "disabled" : "hidden disabled"}>${selectedUploadYear ? `${selectedUploadYear}학년도 파일 확인` : "파일 확인"}</button>
    </form>`;
    ...
    root.innerHTML = `
      ...
      <div class="school-integration-layout">
        <section class="admin-card school-upload-card" aria-busy="${state.curriculumBusy}">
          ...
          <div class="curriculum-entry-methods">
            ...
            <button class="upload-zone curriculum-upload-zone ${state.curriculumBusy ? "is-busy" : ""}" type="button" data-request-curriculum-upload ${state.curriculumBusy ? "disabled" : ""}>
              ...
              <small>입학년도별 엑셀·한글 편제표를 등록합니다.</small>
            </button>
          </div>
        ...
        <aside class="admin-card connected-schools-card">
          <div class="admin-section-head"><div><p class="section-kicker">CONNECTED SCHOOLS</p><h2>연동 학교 목록</h2></div><button class="connected-school-list-trigger" type="button" data-open-connected-school-list><strong>${state.schools.length.toLocaleString("ko-KR")}개 학교</strong>${icon("arrow")}</button></div>
  ...
  root.addEventListener("click", async (event) => {
    ...
    const recommendCourse = event.target.closest("[data-recommend-course]");
    if (recommendCourse) {
      openCourseGuideByName(recommendCourse.dataset.recommendCourse || "");
      return;
    }

    const simulationCourse = event.target.closest("[data-simulation-course]");
    if (simulationCourse) {
      openCourseGuideByName(simulationCourse.dataset.simulationCourse || "");
      return;
    }
    ...
    if (recommendDepartmentChoice) {
      ...
      const departmentOptions = recommendDepartmentChoice.closest(".recommend-department-options") || root.querySelector(".recommend-department-options");
      const departmentScrollTop = preserveDepartmentScroll
        ? departmentOptions?.scrollTop || 0
        : 0;
    ...
    if (event.target.closest("[data-open-connected-school-list]")) {
    ...
    if (schoolCard && schoolStore) {
      openHeaderSchoolPicker(schoolCard.dataset.schoolId);
      return;
    }

    if (event.target.closest("[data-school-course-toggle]")) {
      if (!state.curriculum) {
        const headerPicker = document.querySelector(".header-school-picker");
        const headerTrigger = headerPicker?.querySelector("[data-school-trigger]");
        const reduceMotion = window.matchMedia?.("(prefers-reduced-motion: reduce)").matches;
        window.scrollTo({ top: 0, behavior: reduceMotion ? "auto" : "smooth" });
        headerTrigger?.focus({ preventScroll: true });
        openHeaderSchoolPicker();
    ...
    if (event.target.closest("[data-semester-lock-info]")) {
      showCurriculumAlert(
        "2학기는 아직 선택할 수 없습니다",
        "1학기 선택을 완료하면 2학기 선택 옵션이 열립니다.",
        "학기 선택 안내"
      );
      return;
    }
    ...
    if (simulationSchoolOption && schoolStore) {
      state.simulationSchoolSearch = "";
      const simulationMenu = root.querySelector("[data-simulation-school-menu]");
      if (simulationMenu) simulationMenu.hidden = true;
      root.querySelector("[data-open-school-picker]")?.setAttribute("aria-expanded", "false");
      openHeaderSchoolPicker(simulationSchoolOption.dataset.simulationSchoolId);
      return;
    }
    ...
    const uploadYearButton = event.target.closest("[data-upload-year-select]");
    if (uploadYearButton) {
      const admissionYear = Number(uploadYearButton.value);
      if (!SUPPORTED_ADMISSION_YEARS.includes(admissionYear)) return;
      state.schoolAuthUploadYear = admissionYear;
      root.querySelectorAll("[data-upload-year-select]").forEach((button) => {
        const selected = Number(button.value) === admissionYear;
      ...
      const selectedFile = targetSlot?.querySelector("[data-auth-curriculum-file]")?.files?.[0];
      const submitButton = root.querySelector("[data-upload-year-submit]");
      if (submitButton) {
        submitButton.hidden = false;
        submitButton.disabled = !selectedFile;
        submitButton.textContent = `${admissionYear}학년도 파일 확인`;
      }
      requestAnimationFrame(() => targetSlot?.querySelector("input[type='file']")?.focus({ preventScroll: true }));
      return;
    }

    const editUploadedCurriculumButton = event.target.closest("[data-edit-uploaded-curriculum]");
    if (editUploadedCurriculumButton) {
      const admissionYear = Number(editUploadedCurriculumButton.dataset.editUploadedCurriculum);
      const schoolName = completeSchoolName(state.schoolAuthSchoolName);
      const school = state.schools.find((item) => (
        normalizedKey(item.name) === normalizedKey(schoolName)
        && (!state.schoolAuthRegion || item.region === state.schoolAuthRegion)
      ));
      if (!school || !SUPPORTED_ADMISSION_YEARS.includes(admissionYear) || !schoolStore?.loadCurriculumForCopy) {
        showCurriculumAlert("편제표를 불러오지 못했습니다", "등록된 학교와 입학년도를 다시 확인해 주세요.", "편제표 수정 오류");
        return;
      }
      const originalText = editUploadedCurriculumButton.textContent;
      editUploadedCurriculumButton.disabled = true;
      editUploadedCurriculumButton.textContent = "불러오는 중";
      try {
        const storedCurriculum = await schoolStore.loadCurriculumForCopy({
          schoolId: school.id,
          schoolName: school.name,
          region: school.region,
          admissionYear
        });
        const pendingCurriculum = createStoredCurriculumImport(storedCurriculum, school);
        if (!pendingCurriculum) throw new Error("등록된 편제표를 찾을 수 없습니다.");
        closeSchoolAuthDialog();
        state.pendingCurriculumIdentity = { schoolName: school.name, region: school.region };
        state.pendingCurriculum = pendingCurriculum;
        state.curriculumPreviewIndex = 0;
        state.curriculumPreviewGradeIndex = 0;
        state.curriculumYearPickerOpen = false;
        state.curriculumRegionPickerOpen = false;
        state.curriculumCoursePicker = null;
        state.curriculumImportMessage = `${school.name} ${admissionYear}학년도 편제표를 불러왔습니다.`;
        resetCurriculumDraftState({ clearIdentity: false });
        preparePendingCurriculumForEditing();
        markPendingCurriculaSaved();
        renderAdmin();
        ...
        console.error("등록된 편제표 불러오기 실패:", error);
        showCurriculumAlert("편제표를 불러오지 못했습니다", localizedAccessError(error, "upload"), "편제표 수정 오류");
        if (editUploadedCurriculumButton.isConnected) {
          editUploadedCurriculumButton.disabled = false;
          editUploadedCurriculumButton.textContent = originalText;
        }
      }
      return;
    }

    if (event.target.closest("[data-load-upload-curriculum-draft]")) {
      const draft = state.uploadCurriculumDraft;
      const identity = state.pendingCurriculumIdentity;
    ...
    if (event.target.closest("[data-curriculum-year-toggle]")) {
      state.curriculumYearPickerOpen = !state.curriculumYearPickerOpen;
      const toggle = root.querySelector("[data-curriculum-year-toggle]");
      const options = root.querySelector(".curriculum-year-options");
      toggle?.setAttribute("aria-expanded", String(state.curriculumYearPickerOpen));
      if (options) options.hidden = !state.curriculumYearPickerOpen;
      return;
    }

    const curriculumYearOption = event.target.closest("[data-curriculum-year-option]");
    if (curriculumYearOption) {
      const currentIndex = state.curriculumPreviewIndex;
      const nextIndex = Number(curriculumYearOption.dataset.curriculumYearOption);
      state.curriculumYearPickerOpen = false;
      const toggle = root.querySelector("[data-curriculum-year-toggle]");
      const options = root.querySelector(".curriculum-year-options");
      toggle?.setAttribute("aria-expanded", "false");
      if (options) options.hidden = true;
      if (!Number.isInteger(nextIndex) || !pendingCurriculumItems()[nextIndex] || nextIndex === currentIndex) return;
      await requestCurriculumLeave(() => {
        state.curriculumPreviewIndex = nextIndex;
        state.curriculumPreviewGradeIndex = 0;
        state.curriculumYearPickerOpen = false;
        state.curriculumRegionPickerOpen = false;
        state.curriculumCoursePicker = null;
    ...
    const copyCurriculumYearButton = event.target.closest("[data-copy-curriculum-year]");
    if (copyCurriculumYearButton) {
      const targetIndex = Number(copyCurriculumYearButton.dataset.curriculumIndex);
      const targetCurriculum = pendingCurriculumItems()[targetIndex];
      const copyTools = copyCurriculumYearButton.closest(".curriculum-copy-tools");
      const sourceSelect = copyTools?.querySelector("[data-curriculum-copy-source]");
      const [sourceKind, sourceValue] = String(sourceSelect?.value || "").split(":");
      if (!targetCurriculum || !sourceKind || !sourceValue) {
        showToast("불러올 입학년도 편제표를 선택해 주세요.");
        return;
      }
      const sourceLabel = sourceSelect.selectedOptions?.[0]?.textContent || `${sourceValue}년 입학생`;
      if (targetCurriculum.courseCount && !confirm(`${sourceLabel}의 1·2·3학년 편제로 현재 ${targetCurriculum.admissionYear}년 입학생 편제를 교체할까요?`)) return;
      const originalText = copyCurriculumYearButton.textContent;
      copyCurriculumYearButton.disabled = true;
      copyCurriculumYearButton.textContent = "불러오는 중";
      try {
        let sourceCurriculum = null;
        if (sourceKind === "pending") sourceCurriculum = pendingCurriculumItems()[Number(sourceValue)] || null;
        else if (sourceKind === "published") {
          if (!schoolStore?.loadCurriculumForCopy) throw new Error("저장된 편제표 불러오기 기능을 사용할 수 없습니다.");
          const linkedSchool = linkedSchoolForCurriculum(targetCurriculum);
          sourceCurriculum = await schoolStore.loadCurriculumForCopy({
            schoolId: linkedSchool?.id,
            schoolName: targetCurriculum.schoolName,
            region: targetCurriculum.region,
            admissionYear: Number(sourceValue)
          });
        }
        if (!sourceCurriculum || !copyCurriculumStructure(targetCurriculum, sourceCurriculum)) {
          throw new Error(`${sourceValue}년 입학생의 저장된 3개년 편제를 찾지 못했습니다.`);
        }
        preparePendingCurriculumForEditing();
        state.curriculumImportMessage = `${sourceValue}년 입학생의 3개년 편제를 ${targetCurriculum.admissionYear}년 입학생 편제로 복사했습니다.`;
        refreshCurriculumPreviewSelectionInPlace({ pageChanged: true });
        showToast(state.curriculumImportMessage, 4200);
        requestAnimationFrame(() => root.querySelector(`[data-copy-curriculum-year][data-curriculum-index="${targetIndex}"]`)?.focus({ preventScroll: true }));
      } catch (error) {
        console.error("다른 입학년도 편제 불러오기 실패:", error);
        showCurriculumAlert("편제표를 불러오지 못했습니다", localizedAccessError(error, "publish"), "입학년도 편제 복사 오류");
        copyCurriculumYearButton.disabled = false;
        copyCurriculumYearButton.textContent = originalText;
      }
      return;
    }
    ...
    if (curriculumPickerCategory && state.curriculumCoursePicker) {
      ...
      root.querySelectorAll("[data-curriculum-picker-category]").forEach((button) => {
        button.classList.toggle("is-selected", button.dataset.curriculumCoursePickerCategory === state.curriculumCoursePickerCategory);
      });
      refreshCurriculumCoursePickerInPlace({ resultsOnly: true });
      return;
    }

    const toggleCurriculumCustomCourse = event.target.closest("[data-toggle-curriculum-custom-course]");
    if (toggleCurriculumCustomCourse && state.curriculumCoursePicker) {
      state.curriculumCoursePicker.customEntryOpen = !state.curriculumCoursePicker.customEntryOpen;
      refreshCurriculumCoursePickerInPlace({ customOnly: true, focusCustom: state.curriculumCoursePicker.customEntryOpen });
      return;
    }
    ...
    if (event.target.closest("[data-confirm-curriculum-course-picker]") && state.curriculumCoursePicker) {
      ...
      const customCourseKeys = new Set((picker.customCourses || []).map(curriculumCourseAliasKey).filter(Boolean));
      const selectedCustomCourses = selectedNames.filter((course) => customCourseKeys.has(curriculumCourseAliasKey(course)));
    ...
    if (addCurriculumOption) {
      ...
      const semesterEditor = addCurriculumOption.closest("[data-curriculum-semester-editor]");
      const optionsEditor = semesterEditor?.querySelector(".curriculum-options-editor");
      const optionElement = curriculumMarkupElement(curriculumOptionEditorMarkup(
        Number(addCurriculumOption.dataset.curriculumIndex),
        context.grade.grade,
        context.semester.semester,
        option,
        optionNumber - 1
      ));
      if (!semesterEditor || !optionsEditor || !optionElement) return;
      context.semester.options.push(option);
      optionsEditor.querySelector(".curriculum-no-options")?.remove();
      optionsEditor.append(optionElement);
      refreshCurriculumEditorSummaries();
      const semesterSummary = semesterEditor.querySelector("[data-curriculum-semester-summary]");
      const electiveSummary = semesterEditor.querySelector("[data-curriculum-elective-summary]");
      if (semesterSummary) semesterSummary.textContent = `공통·학교 지정 ${context.semester.common.length} · 선택 옵션 ${context.semester.options.length}`;
      if (electiveSummary) electiveSummary.textContent = `개별 ${context.semester.standalone.length}과목 · 옵션 ${context.semester.options.length}개`;
      requestAnimationFrame(() => {
        optionElement.scrollIntoView({ block: "nearest" });
        const optionInput = optionElement.querySelector("[data-curriculum-option-label]");
    ...
    if (event.target.closest("[data-clear-curriculum-preview]")) {
      await requestCurriculumLeave(closeCurriculumPreview);
      return;
    }
    ...
    if (saveCurriculumDraftButton) {
      ...
      try {
        ...
        const savedAt = Number.isNaN(savedDate.getTime()) ? "" : ` 저장 시각은 ${savedDate.toLocaleString("ko-KR")}입니다.`;
        showCurriculumAlert(
          "임시저장 완료",
          `${savedDraft.storage === "local" ? "현재 편제표를 이 브라우저에 임시저장했습니다." : "현재 편제표를 안전하게 임시저장했습니다."}${savedAt}`,
          "저장 완료",
          "success"
        );
      } catch (error) {
    ...
    const publishCurriculumButton = event.target.closest("[data-publish-curriculum]");
    if (publishCurriculumButton) {
      if (!state.pendingCurriculum || !schoolStore) return;
      let pendingCurricula;
      try {
        pendingCurricula = validatePendingCurriculumEdits([state.curriculumPreviewIndex]);
      } catch (error) {
        const validationMessage = error.message || "학교명, 지역, 입학년도와 각 선택 옵션을 확인해 주세요.";
        state.curriculumImportMessage = `수정 내용을 확인해 주세요. ${validationMessage}`;
      ...
      try {
        ...
        showCurriculumAlert(
          ...
          "success",
          continueUploadFlow
            ? () => returnToTeacherCurriculumUpload({ schoolName, region })
            : () => requestCurriculumLeave(closeCurriculumPreview)
        );
      } catch (error) {
        console.error("학교 편제표 공개 실패:", error);
        const publishErrorMessage = localizedAccessError(error, "publish");
        state.curriculumImportMessage = `저장하지 못했습니다. ${publishErrorMessage}`;
        showCurriculumAlert("편제표를 저장하지 못했습니다", publishErrorMessage, "편제표 저장 오류");
      } finally {
        state.curriculumBusy = false;
        if (publishCurriculumButton.isConnected) {
          publishCurriculumButton.disabled = false;
          publishCurriculumButton.textContent = originalPublishText;
        }
        if (draftButton?.isConnected) draftButton.disabled = false;
        updateChrome();
      }
      return;
    }
  ...
  root.addEventListener("submit", async (event) => {
    const customCourseForm = event.target.closest("[data-curriculum-custom-course-form]");
    if (customCourseForm) {
      event.preventDefault();
      const picker = state.curriculumCoursePicker;
      const input = customCourseForm.querySelector("input[name='customCourse']");
      const enteredName = compactText(input?.value);
      if (!picker || !input || !enteredName) {
        showToast("직접 추가할 과목명을 입력해 주세요.");
        input?.focus();
        return;
      }
      const reference = curriculumCourseReference(enteredName);
      const courseName = reference?.name || enteredName;
      const courseKey = curriculumCourseAliasKey(courseName);
      if (!courseKey) {
        showToast("과목명을 확인해 주세요.");
        input.focus();
        input.select();
        return;
      }
      const selected = new Set(picker.selectedKeys || []);
      if (selected.has(courseKey)) {
        showToast("이미 선택한 과목입니다.");
        input.focus();
        input.select();
        return;
      }
      selected.add(courseKey);
      picker.selectedKeys = [...selected];
      if (!reference) picker.customCourses = uniqueCourseNames([...(picker.customCourses || []), courseName]);
      input.value = "";
      if (!reference) {
        state.curriculumCoursePickerSearch = "";
    ...
    if (button) {
      ...
      button.textContent = teacherSchoolForm ? "학교 확인 중" : teacherUploadForm ? "업로드 확인 중" : "로그인 중";
    }
    try {
      ...
      if (teacherSchoolForm) {
        ...
        const rawSchoolName = compactText(formData.get("schoolName"));
        const region = compactText(formData.get("region"));
        state.schoolAuthSchoolName = rawSchoolName;
        if (!/^.+고등학교$/u.test(rawSchoolName)) throw new Error("OO 고등학교 형식으로 작성해 주세요");
        const schoolName = completeSchoolName(rawSchoolName);
  ...
  root.addEventListener("change", async (event) => {
    ...
    if (event.target.matches("[data-auth-curriculum-file]")) {
      ...
      const extension = String(file?.name || "").split(".").pop().toLocaleLowerCase("en");
      if (file && !["xlsx", "xls", "hwp", "hwpx"].includes(extension)) {
        event.target.value = "";
        file = null;
        showCurriculumAlert("업로드 파일을 확인해 주세요", "엑셀(.xlsx, .xls) 또는 한글(.hwp, .hwpx) 파일만 업로드할 수 있습니다.", "파일 형식 안내");
      }
      ...
      if (status) {
        ...
        status.textContent = file ? "파일 선택됨" : isLinked ? "업로드 완료" : "업로드 전";
      }
      const submitButton = root.querySelector("[data-upload-year-submit]");
      if (submitButton && Number(state.schoolAuthUploadYear) === Number(event.target.dataset.uploadAdmissionYear)) {
        submitButton.hidden = false;
    ...
    if (event.target.matches("[data-curriculum-school-name]")) {
      const rawSchoolName = compactText(event.target.value);
      if (!/^.+고등학교$/u.test(rawSchoolName)) {
        const currentSchoolName = compactText(pendingCurriculumItems()[0]?.schoolName || state.pendingCurriculum?.schoolName);
        event.target.value = currentSchoolName;
        showCurriculumAlert("학교명 형식을 확인해 주세요", "OO 고등학교 형식으로 작성해 주세요", "학교명 입력 안내");
        return;
      }
      const schoolName = completeSchoolName(rawSchoolName);
  ...
  document.addEventListener("click", async (event) => {
    const navigationButton = event.target.closest("[data-nav-href], .app-brand");
    if (navigationButton) {
      event.preventDefault();
    ...
    const majorBookUniversities = event.target.closest("#record-dialog [data-major-book-universities]");
    if (majorBookUniversities) {
      const departmentId = majorBookUniversities.dataset.departmentId;
      const bookIndex = Number(majorBookUniversities.dataset.bookIndex);
      const isExpanded = state.dialogDepartmentId === departmentId && state.dialogBookIndex === bookIndex;
      state.dialogDepartmentId = departmentId;
      state.dialogBookIndex = isExpanded ? -1 : bookIndex;
      openDepartment(departmentId, { keepBook: true });
      if (!isExpanded) requestAnimationFrame(() => detailContent.querySelector(".major-book-university-reveal")?.scrollIntoView({ behavior: "smooth", block: "nearest" }));
      return;
    }

    const majorCourse = event.target.closest("#record-dialog [data-major-course]");
    if (majorCourse) {
      openCourseGuideByName(majorCourse.dataset.majorCourse || "");
      return;
    }

    const majorSubject = event.target.closest("#record-dialog [data-major-subject-universities]");
    if (majorSubject) {
      const isExpanded = state.dialogDepartmentId === majorSubject.dataset.departmentId
      ...
      state.dialogSubjectKind = isExpanded ? "" : majorSubject.dataset.subjectKind;
      state.dialogSubjectName = isExpanded ? "" : majorSubject.dataset.subjectName;
    ...
    if (event.target === menu || event.target.closest(".header-school-picker [data-school-menu-close]")) {
      closeHeaderSchoolPicker();
      return;
    }
    ...
    if (schoolOption && schoolStore) {
      openHeaderSchoolPicker(schoolOption.dataset.schoolId);
      return;
    }
  });

  document.querySelector(".header-school-picker [data-school-menu]")?.addEventListener("close", () => {
    document.querySelector(".header-school-picker [data-school-trigger]")?.setAttribute("aria-expanded", "false");
    state.schoolPickerPendingId = "";
    renderHeaderSchoolPicker();
  });
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (curriculumLeaveDialog?.open || curriculumAlertDialog?.open) return;
    ...
    if (state.pendingCurriculum && root.querySelector("[data-curriculum-preview-overlay]")) {
      ...
      void requestCurriculumLeave(closeCurriculumPreview);
      return;
    }
    closeHeaderSchoolPicker();
  });
  ...
  curriculumAlertDialog?.addEventListener("click", async (event) => {
    ...
    if (!event.target.closest("[data-curriculum-alert-close]")) return;
    const confirmAction = curriculumAlertConfirmAction;
    curriculumAlertConfirmAction = null;
    curriculumAlertDialog.close();
    if (confirmAction) await confirmAction();
  });

  curriculumLeaveDialog?.addEventListener("cancel", (event) => {
    event.preventDefault();
    curriculumLeaveConfirmAction = null;
    curriculumLeaveDialog.close();
  });

  curriculumLeaveDialog?.addEventListener("click", async (event) => {
    if (event.target === curriculumLeaveDialog || event.target.closest("[data-cancel-curriculum-leave]")) {
      curriculumLeaveConfirmAction = null;
      curriculumLeaveDialog.close();
      return;
    }
    if (!event.target.closest("[data-confirm-curriculum-leave]")) return;
    const confirmAction = curriculumLeaveConfirmAction;
    curriculumLeaveConfirmAction = null;
    curriculumLeaveDialog.close();
    if (confirmAction) await confirmAction();
  });
  ...
  window.addEventListener("beforeunload", (event) => {
    if (!pendingCurriculumHasUnsavedChanges()) return;
    event.preventDefault();
    event.returnValue = "";
  });
  ...
  window.DatabaseApp = {
    ...
    markPendingCurriculaSaved,
    pendingCurriculumHasUnsavedChanges,
    renderRecommend,
