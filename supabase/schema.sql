...
-- 편제표를 공개하기 전에 마지막 작업 상태를 학교별로 보관합니다.
-- 등록 비밀번호는 Supabase Auth에서만 확인하며 이 테이블에는 저장하지 않습니다.
create table if not exists public.curriculum_drafts (
  id uuid primary key default gen_random_uuid(),
  school_name text not null constraint curriculum_drafts_school_name_check check (btrim(school_name) ~ '.+고등학교$'),
  region text not null constraint curriculum_drafts_region_check check (region in (
    '서울특별시', '부산광역시', '대구광역시', '인천광역시', '광주광역시', '대전광역시', '울산광역시',
    '세종특별자치시', '경기도', '강원특별자치도', '충청북도', '충청남도', '전북특별자치도', '전라남도',
    '경상북도', '경상남도', '제주특별자치도'
  )),
  entry_mode text not null default 'manual' check (entry_mode in ('manual', 'upload')),
  data jsonb not null check (jsonb_typeof(data) = 'object'),
...
revoke all on table public.visitor_daily_counts from anon, authenticated;
