...
const ambiguousCourseHeaderRows = [
  ...
  ["선택", "국어", "일반", "화법과 언어", 4, 4, "", "", "", "", "택 3", ""],
  ["선택", "국어", "일반", "독서와 작문", 4, 4, "", "", "", "", "", ""],
  ["선택", "수학", "일반", "대수", 4, 4, "", "", "", "", "", ""],
  ["선택", "수학", "일반", "미적분Ⅰ", 4, 4, "", "", "", "", "", "", ""],
  ["지정", "국어", "공통", "공통국어1", 4, 4, 4, "", "", "", "", ""],
  ["선택", "영어", "융합", "미디어 영어", 4, 4, "", "", 4, "", "", ""]
];
...
window.XLSX = {
  ...
  utils: {
    sheet_to_json(sheet) { return sheet._matrix.map((row) => [...row]); },
    decode_range(reference) {
      const decodeCell = (cell) => {
        const match = String(cell).match(/^([A-Z]+)(\d+)$/i);
        const column = [...match[1].toUpperCase()].reduce((sum, character) => sum * 26 + character.charCodeAt(0) - 64, 0) - 1;
...
async function main() {
  ...
  assert.match(appCss, /\.app-main:has\(> \.recommend-wizard\[data-recommend-step="3"\]\)[\s\S]*?height: auto;[\s\S]*?overflow: visible/);
  assert.match(appCss, /\.recommend-wizard\[data-recommend-step="3"\] \.recommend-course-review[\s\S]*?overflow: visible/);
  assert.match(appCss, /\.recommend-wizard\[data-recommend-step="3"\] \.recommend-common-scope > strong[\s\S]*?font-size: 13px/);
  assert.match(appCss, /\.recommend-wizard\[data-recommend-step="5"\] \.recommend-result-course-group > header[\s\S]*?font-size: 16px/);
  assert.match(appCss, /\.recommend-wizard\[data-recommend-step="4"\] \.recommend-keyword-entry input[\s\S]*?font-size: 16px/);
  assert.match(appCss, /\.recommend-wizard\[data-recommend-step="4"\] \.recommend-special-empty[\s\S]*?font-size: 14px/);
  assert.match(appCss, /\.recommend-wizard\[data-recommend-step="2"\] \.recommend-major-selection > div:nth-child\(2\) p[\s\S]*?max-width: none;[\s\S]*?font-size: 14px;[\s\S]*?white-space: nowrap/);
  assert.match(appCss, /\.recommend-wizard\[data-recommend-step="2"\] \.recommend-major-selection-list > p[\s\S]*?font-size: 14px/);
  assert.match(appCss, /\.subject-toolbar \.search-field[\s\S]*?height: fit-content;[\s\S]*?align-self: center/);
  assert.match(appCss, /\.recommend-wizard\[data-recommend-step="5"\] \.recommend-wizard-head h1[\s\S]*?font-size: clamp\(25px, 4\.2vw, 40px\);[\s\S]*?white-space: nowrap/);
  assert.match(appCss, /\.recommend-wizard\[data-recommend-step="5"\] \.recommend-result-print-row[\s\S]*?margin: 8px 0 14px/);
  ...
  assert.match(appSource, /data-major-course=/);
  assert.match(appSource, /data-major-subject-universities/);
  assert.match(appSource, /data-major-book-universities/);
  assert.match(appSource, /major-book-mobile-universities/);
  assert.match(appSource, /function careerGuideText\(value\)/);
  assert.match(appSource, /function dialogFooterMarkup\(\)/);
  assert.match(appSource, /class="major-dialog-footer dialog-action-footer"><button type="button" data-dialog-close>닫기<\/button>/);
  assert.match(appSource, /class="major-university-hint">대학버튼을 누르면 반영 대학을 확인할 수 있습니다\./);
  assert.match(appSource, /class="common-major-coverage"><b>\$\{subject\.coverageCount\}\/\$\{subject\.totalCount\}<\/b><span>개 학과<\/span>/);
  ...
  assert.match(appSource, />선택 초기화<\/button>/);
  assert.match(appCss, /body:has\(dialog\[open\]\)[\s\S]*?overflow: hidden/);
  assert.match(appCss, /\.sheet-dialog\.is-course-dialog[\s\S]*?overflow-y: auto;[\s\S]*?overscroll-behavior: contain/);
  assert.match(appCss, /\.recommend-wizard\[data-recommend-step="2"\] \.recommend-department-options[\s\S]*?height: clamp\(220px, 34dvh, 280px\)/);
  assert.match(appCss, /\.sheet-dialog\.is-major-dialog \.major-subject-list[\s\S]*?grid-template-columns: repeat\(2, minmax\(0, 1fr\)\)/);
  ...
  assert.deepEqual(ambiguousHeaderResult.curricula[0].grades[0].semesters[0].common, ["공통국어1"]);
  assert.deepEqual(ambiguousHeaderResult.curricula[0].grades[1].semesters[0].electives, ["미디어 영어"]);
  ...
  assert.match(root.innerHTML, /다음 화면에서 업로드할 신입생 입학년도를 한 개 선택합니다/);
