...
$synonymGroups = [ordered]@{
    K028 = @{ Canonical='진로 없음'; Weight=5.5; Values=@('진로가 없다','진로를 못 정했다','진로가 안 정해졌다','진로 미정','진로가 불확실','진로가 확실하지 않','진로가 애매','진로가 모호','진로가 정해지지 않','진로를 아직 못 정','진로를 결정하지 못','진로가 고민','진로 때문에 고민','진로를 고민 중','진로가 막막','진로를 모르','진로 방향을 모르','진로가 헷갈','진로 선택이 고민','진로 선택을 고민') }
